package com.ventarapida.api_gateway.filter;
...
public class AuthenticationFilter extends AbstractGatewayFilterFactory<AuthenticationFilter.Config>{
    ...
    public GatewayFilter apply(Config config){
        return (exchange, chain) ->{
            ...
            try {
                //validar token
                Jwts.parserBuilder().setSigningKey(Keys.hmacShaKeyFor(secreto.getBytes(StandardCharsets.UTF_8))).build().parseClaimsJws(token);
            } catch (Exception e) {
                //token expirado o firma erronea
